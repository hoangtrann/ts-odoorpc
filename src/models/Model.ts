...
export class Model<T = any> {
   ...
   * ```
   */
  async search(domain: Domain = [], options?: SearchOptions): Promise<RecordSet<T>> {
  ...
  async searchRead(
    ...
    for (const record of data) {
      if (record && typeof record === 'object' && 'id' in record) {
        (recordset as any).cache.set(record.id, record);
      }
    }
   ...
   * ```
   */
  async create(values: Partial<T>): Promise<RecordSet<T>> {
   ...
   * ```
   */
  async write(ids: number[], values: Partial<T>): Promise<boolean> {
    if (ids.length === 0) {
      return true;
    }
  ...
  browse(ids: number | number[]): RecordSet<T> {
    ...
    return new RecordSet<T>(this, idArray);
  }
  ...
  withContext(ctx: Record<string, any>): Model<T> {
    ...
    return newEnv.model<T>(this.modelName);
  }
  ...
  withSchema(schema: any): Model<T> {
    return new Model<T>(this.env, this.modelName, {
      ...this.options,
      schema,
    });
  }
   ...
   * ```
   */
  async call(method: string, args: any[] = [], kwargs?: Record<string, any>): Promise<any> {
   ...
   * ```
   */
  async fieldsGet(fields?: string[]): Promise<FieldsGetResult> {
    const kwargs: Record<string, any> = {};
